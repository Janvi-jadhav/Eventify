...
export default function PreferencesArts() {
  const navigate = useNavigate()
  const [selectedTypes, setSelectedTypes] = useState([])

  const artTypes = [
    'Painting', 'Sculpture', 'Photography', 'Design', 'Theater',
    'Dance', 'Film', 'Literature', 'Crafts', 'Installation Art'
  ]
  ...
  const handleSave = () => {
    localStorage.setItem('artsPreferences', JSON.stringify(selectedTypes))
    navigate('/page2')
  }
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl p-10 shadow-lg border border-slate-100 dark:border-slate-700">
          <div className="mb-8">
            <div className="flex items-center gap-4 mb-4">
              ...
              <h1 className="text-4xl font-extrabold text-slate-900 dark:text-slate-100">Arts & Culture Preferences</h1>
            </div>
            <p className="text-slate-600 dark:text-slate-400 text-lg">Select your favorite art forms to personalize your event feed</p>
          </div>

          <div className="mb-12">
            <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100 mb-6">Art Forms</h2>
